...
def process_question(transcribed_text):
    ...
    if match:
        ...
        if question_type == 'Kto nakręcił' or question_type == 'Reżyser filmu' or question_type == 'Kto jest reżyserem':
            ...
            find = movie.details(nr_id)
            find=find['vote_average']
            print(str(find) + "/10")
            find = num2words(find, lang='pl') + "na 10"
...
if __name__ == "__main__":
    ...
    engine.setProperty('voice', engine.getProperty('voices')[2].id) # w moim systemie indeks [2] odpowiada polskiemu glosowi, byc moze konieczne jest na innym komputerze wybranie innego indeksu. W tym celu nalezy uruchomic kod w linijkach 113-114 i zobaczyc ktory pakiet odpowiada jezykowi polskiemu.
    while True:
